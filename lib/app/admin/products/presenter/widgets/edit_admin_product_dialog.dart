...
final class EditAdminProductDialog extends StatefulWidget {
  final AdminProductModel product;

  const EditAdminProductDialog({required this.product, super.key});

  @override
  State<EditAdminProductDialog> createState() => _EditAdminProductDialogState();
...
class _EditAdminProductDialogState extends State<EditAdminProductDialog> {
  ...
  void initState() {
    ...
    picture = widget.product.picture.isNotEmpty ? widget.product.pictureBytes : null;
    _nameController.text = widget.product.name;
    _valueController.text = (widget.product.value / 100).toStringAsFixed(2).replaceAll('.', ',');
    if (widget.product is AdminProductDrinkModel) {
      final drink = widget.product as AdminProductDrinkModel;
      selectedProductTypeValue = 0;
      selectedDrinkType = drink.type.name.toUpperCase();
      _volumeController.text = drink.volume.toString();
    } else {
      final food = widget.product as AdminProductFoodModel;
      selectedProductTypeValue = 1;
      selectedFoodSize = food.size.name.toUpperCase();
...
final class EditProductImage extends StatelessWidget {
  final Uint8List? picture;
  final void Function(Uint8List) onSelectFile;

  const EditProductImage({required this.onSelectFile, super.key, this.picture});
